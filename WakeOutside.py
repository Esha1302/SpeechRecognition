...
while (True and p != 0): #as voice assistant keeps looking for the occcurance of the wake word
    ...
    if result == 0: #if file isn't found
        playsound("file not found message", datao) #giving user the desired message
        continue
        ...
        while (True):
            ...
            try: #if it's able to convert wake word into string without error
                ...
                if (datao['wake']['keyphrase'] in text1): #if wake word is present in text1
                    playsound("upordown message", datao) #tells user to choose whether he wants to go up or down
                    while (k == 0):
                        audio2 = takeinput(datao) #to take input of his choice
                        try: #if it's able to recognize choice without error
                            text2 = recognizeSphinx(audio2) #stored result in text2
                            ...
                            if 'up' in text2 or text2 in datao['up']: #if choice is up
                                ...
                                playsound("choice again message", datao) #tells user to make the choice again

                        except sr.UnknownValueError as e: #if sphinx is unable to recognize choice
                            logger(e) #add error in txt file
                            playsound("error message", datao) #playing error message
                            playsound("choice again message", datao) #tells user to make the choice again
                    ...
                    playsound("choice again message", datao) #tells user to make choice again
